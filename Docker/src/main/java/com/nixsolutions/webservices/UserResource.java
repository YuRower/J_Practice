...
public class UserResource {
    ...
    public ApiUserResponse<User> get(@PathParam("login") String login) {
        ...
        if (user == null) {
            return new ApiUserResponse<User>(HttpStatus.NOT_FOUND.value(),
                    "User not found", user);
        }
        return new ApiUserResponse<User>(HttpStatus.OK.value(),
                "User successfully found by id", user);
    }
    ...
    public ApiUserResponse<User> create(User user) {
        if (userService.isUserLoginUnique(user.getLogin())) {
            return new ApiUserResponse(HttpStatus.CONFLICT.value(),
                    "user already exists", user);
        }
        LOGGER.debug("User -- > {} ,and role {}", user, user.getRoleId());

        userService.saveUser(user);
        return new ApiUserResponse<User>(HttpStatus.CREATED.value(),
    ...
    public ApiUserResponse<User> update(@PathParam("login") String login,
            User newUser) {

        boolean userExist = userService.isUserLoginUnique(login);
        if (userExist == false) {
            return new ApiUserResponse(HttpStatus.BAD_REQUEST.value(),
                    "user not found", newUser);
        }

        LOGGER.debug("------------------------User for upadte -------- > {}",
                newUser);
        userService.updateUser(newUser);
        return new ApiUserResponse(HttpStatus.NO_CONTENT.value(),
                "User updated successfully", newUser);
    ...
    public ApiUserResponse<User> delete(@PathParam("login") String login) {
        ...
        LOGGER.debug("------------------------User for delete -------- > {}",
                user);

        userService.delete(user);
        return new ApiUserResponse(HttpStatus.ACCEPTED.value(),
