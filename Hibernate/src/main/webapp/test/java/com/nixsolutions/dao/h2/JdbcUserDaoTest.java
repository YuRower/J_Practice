...
import com.nixsolutions.dao.h2.jdbc.JdbcUserDao;
...
public class JdbcUserDaoTest extends DBUnitConfig {
	...
	public void setUp() throws Exception {
		...
		User user1 = new User(1l, "login", "password", "email@com", "Ivan", "Ivanov", date, role1.getRoleId());

		User user2 = new User(2l, "login2", "password2", "email2@com", "Petr", "Petrov", date1, role2.getRoleId());

		User user3 = new User(3l, "login3", "password3", "email3@com", "Fedor", "Fedorov", date2, role1.getRoleId());

		userforUpdate = new User(3l, "login3", "password4", "email4@com", "Fedor", "Fedorov", date2, role1.getRoleId());
		users[0] = user1;
		users[1] = user2;
		users[2] = user3;
	}
	...
	public void testCreate() throws DatabaseUnitException {
		...
		ITable expected = dbTestHelper.getTableFromFile(TABLE_NAME, afterCreate);
	...
	public void testUpdate() throws DatabaseUnitException {
		...
		ITable expected = dbTestHelper.getTableFromFile(TABLE_NAME, afterUpdate);
		ITable actual = dbTestHelper.getTableFromSchema(TABLE_NAME);
